...
IMPORTANT CHANGES:
- Stores study_id and series_id as STRINGS (not integers)
- Includes 'files' array with filenames for each series
- Properly formats data for JavaScript consumption
...
class DicomUploader:
    ...
    def upload_series(self, study_id, series_id, series_dir):
        ...
        for idx, dcm_file in enumerate(dcm_files, 1):
            ...
            # Track file info - JUST filename for JavaScript
            file_list.append({
        ...
        # IMPORTANT: Return series_id as STRING
        return {
            'series_id': str(series_id),  # STRING not integer!
            'description': first_dcm_metadata['series_description'],
            'slice_count': len(dcm_files),
            'files': file_list,  # Array of {filename, instance_number, slice_location}
    ...
    def upload_study(self, study_dir):
        ...
        study_id = study_dir.name  # Keep as string
        ...
        for series_dir in sorted(series_dirs):
            series_id = series_dir.name  # Keep as string
        ...
        # IMPORTANT: Store study_id as STRING to match JavaScript expectations
        study_metadata = {
            'study_id': str(study_id),  # STRING not integer
            'series': series_list,
        ...
        # Save to Firestore using study_id as document ID
        doc_ref = self.db.collection('studies').document(str(study_id))
        doc_ref.set(study_metadata)
        
        print(f"  💾 Saved to Firestore: studies/{study_id}")
    ...
    def upload_studies(self, dicom_root, trial_mode=False, max_studies=None):
        ...
        print("\n" + "="*70)
        print("🔍 VERIFY YOUR DATA IN FIRESTORE")
        print("="*70)
        print("Go to: https://console.firebase.google.com/project/lstv-annotation-tool/firestore")
        print("\nYour studies should look like:")
        print("""
{
  "study_id": "4003253",  // STRING not number!
  "series": [
    {
      "series_id": "2448190387",  // STRING not number!
      "description": "T2",
      "slice_count": 43,
      "files": [
        {"filename": "1.dcm", ...},
...
def main():
    ...
    print("\n🎉 All done!")
    print("\n" + "="*70)
    print("⚠️  CRITICAL: YOU MUST FIX CORS BEFORE THE APP WILL WORK!")
    print("="*70)
    print("\nRun this command to fix CORS:")
    print(f"  gsutil cors set cors.json gs://{uploader.bucket.name}")
    print("\nSee CORS_FIX_INSTRUCTIONS.md for details")
    print("\nNext steps:")
    print("1. Fix CORS (see above)")
    print("2. Verify files in Firebase Console → Storage")
    print("3. Check metadata in Firebase Console → Firestore")
    print("4. Test your app at: https://gregory-schwing-md-phd.github.io/lstv-annotation-tool/")
